...
void setupWifiList() {
  ...
  if (!file) {
    ...
    networks[0].id = "Radiosonde";
    networks[0].pw = "Radiosonde";
    return;
  }
  int i = 0;
...
void addSondeStatus(char *ptr, int i)
{
  ...
  sprintf(ptr + strlen(ptr), "<tr><td id=\"sfreq\">%3.3f MHz, Type: %s</td><tr><td>ID: %s", s->freq, sondeTypeStr[s->type],
          s->validID ? s->id : "<?""?>");
  if (s->validID && (s->type == STYPE_DFM || s->type == STYPE_DFM06 || s->type == STYPE_DFM09 || s->type == STYPE_M10 || s->type == STYPE_M20)) {
    sprintf(ptr + strlen(ptr), " (ser: %s)", s->ser);
  }
  sprintf(ptr + strlen(ptr), "</td></tr><tr><td>QTHSonde: %.6f,%.6f h=%.0fm</td></tr>\n", s->lat, s->lon, s->alt);
  sprintf(ptr + strlen(ptr), "</td></tr><tr><td>QTHOm: %.6f,%.6f h=%.0fm</td></tr>\n", sonde.config.gps_Lat, sonde.config.gps_Lon, sonde.config.gps_Alt);
  ...
  sprintf(ptr + strlen(ptr), "<tr><td>");
  sprintf(ptr + strlen(ptr), "<a target=\"_empty\" href=\"https://www.google.fr/maps/dir//%.6f,%.6f\">G-MAPS</a> - ", s->lat, s->lon);
...
struct st_configitems config_list[] = {
  ...
  /* KISS tnc settings */
  {"kisstnc.active", "KISS TNC (1 active) (needs reboot)", 0, &sonde.config.kisstnc.active},
  ...
  {"degdec", "Lat,Lon Decimal =0 ou Degres =1)", 0, &sonde.config.degdec}, 
};
...
#if 0
  sonde.clearSonde();
  sonde.addSonde(402.800, STYPE_RS41);
  sonde.addSonde(402.000, STYPE_M10);
  sonde.addSonde(404.800, STYPE_RS41);
  sonde.addSonde(403.000, STYPE_DFM09);
...
void loopDecoder() {
  ...
  //APRSIs
  APRS_IS aprs_is(sonde.config.call, sonde.config.passcode, "RadioSonde", version_id);
  if(!aprs_is.connected())
	{
		Serial.print("connecting to server: ");
		Serial.print(sonde.config.tcpfeed.host);
		Serial.print(" on port: ");
		Serial.println(sonde.config.tcpfeed.port);
		if(!aprs_is.connect(sonde.config.tcpfeed.host, sonde.config.tcpfeed.port, "-t/po"))
		{
